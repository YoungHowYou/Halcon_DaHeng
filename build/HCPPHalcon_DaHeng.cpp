/***********************************************************
 * File generated by the HALCON-Compiler hcomp version 24.11
 * Usage: Interface to C++
 *
 * Software by: MVTec Software GmbH, www.mvtec.com
 ***********************************************************/


#include "HCPPHalcon_DaHeng.h"

#ifndef __APPLE__
#  include "halconcpp/HalconAPI.h"
#else
#  ifndef HC_LARGE_IMAGES
#    include <HALCONCpp/HalconAPI.h>
#  else
#    include <HALCONCppxl/HalconAPI.h>
#  endif
#endif

extern "C" HUserImport int HHalcon_DaHengGetOffset();

int offset = -1;

using namespace HalconCpp;

Hproc_handle PreCallUser(int proc_index_local)
{
  if (offset < 0)
  {
    /* First use of this extension package, query my offset */
    if (0 > (offset = HHalcon_DaHengGetOffset()))
    {
      /*
       * Either the HALCON library was not initialized or the extension package
       * was not loaded. Make sure HALCON is initialized and check the offset
       * again.
       */
      Hproc_handle tmp = HalconAPI::PreCall(0);
      HalconAPI::PostCall(tmp);
      if (0 > (offset = HHalcon_DaHengGetOffset()))
      {
        /* The extension package could not be loaded. */
        HOperatorException::Throw(-1,H_ERR_UEXTNI);
      }
    }
  }
  return HalconAPI::PreCall(offset + proc_index_local);
}




#ifdef CGXSetCommandValue
# error An existing macro name conflicts with CGXSetCommandValue
#endif
HUserExport void HalconCpp::CGXSetCommandValue(const HTuple& CameraHandle, const HTuple& strKey)
{
  Hproc_handle proc = PreCallUser(0);
  HalconAPI::Store(proc,0,CameraHandle);
  HalconAPI::StoreEnc(proc,1,strKey);
  HalconAPI::CallProcedure(proc);
  HalconAPI::PostCall(proc);
}

#ifdef DHGXOPenCameraByID
# error An existing macro name conflicts with DHGXOPenCameraByID
#endif
HUserExport void HalconCpp::DHGXOPenCameraByID(const HTuple& Name, const HTuple& TimeSharingFlicker, const HTuple& CameraType, const HTuple& QueueHandle, HTuple* CameraHandle)
{
  Hproc_handle proc = PreCallUser(1);
  HalconAPI::StoreEnc(proc,0,Name);
  HalconAPI::Store(proc,1,TimeSharingFlicker);
  HalconAPI::Store(proc,2,CameraType);
  HalconAPI::Store(proc,3,QueueHandle);
  HalconAPI::InitOutp(proc,0);
  HalconAPI::CallProcedure(proc);
  HalconAPI::Load(proc,0,CameraHandle);
  HalconAPI::PostCall(proc);
}

#ifdef DHGXCloseLib
# error An existing macro name conflicts with DHGXCloseLib
#endif
HUserExport void HalconCpp::DHGXCloseLib()
{
  Hproc_handle proc = PreCallUser(2);
  HalconAPI::CallProcedure(proc);
  HalconAPI::PostCall(proc);
}

#ifdef DHGXGetLastError
# error An existing macro name conflicts with DHGXGetLastError
#endif
HUserExport void HalconCpp::DHGXGetLastError(HTuple* msg, HTuple* errcode)
{
  Hproc_handle proc = PreCallUser(3);
  HalconAPI::InitOutp(proc,0);
  HalconAPI::InitOutp(proc,1);
  HalconAPI::CallProcedure(proc);
  try
  {
    HalconAPI::LoadEnc(proc,0,msg);
    HalconAPI::Load(proc,1,errcode);
  }
  catch (...)
  {
    if (msg)
      msg->Clear();
    if (errcode)
      errcode->Clear();
    throw;
  }
  HalconAPI::PostCall(proc);
}

#ifdef DHGXInitLib
# error An existing macro name conflicts with DHGXInitLib
#endif
HUserExport void HalconCpp::DHGXInitLib()
{
  Hproc_handle proc = PreCallUser(4);
  HalconAPI::CallProcedure(proc);
  HalconAPI::PostCall(proc);
}

#ifdef DHGXGetLogType
# error An existing macro name conflicts with DHGXGetLogType
#endif
HUserExport void HalconCpp::DHGXGetLogType(HTuple* ui32LogType)
{
  Hproc_handle proc = PreCallUser(5);
  HalconAPI::InitOutp(proc,0);
  HalconAPI::CallProcedure(proc);
  HalconAPI::Load(proc,0,ui32LogType);
  HalconAPI::PostCall(proc);
}

#ifdef DHGXSetLogType
# error An existing macro name conflicts with DHGXSetLogType
#endif
HUserExport void HalconCpp::DHGXSetLogType(const HTuple& ui32LogType)
{
  Hproc_handle proc = PreCallUser(6);
  HalconAPI::Store(proc,0,ui32LogType);
  HalconAPI::CallProcedure(proc);
  HalconAPI::PostCall(proc);
}

